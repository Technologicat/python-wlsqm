...
cimport wlsqm.wlsqm2.wlsqm2_poly as poly # Taylor expansion and general polynomial evaluation
...
# diff  : in, one of the i1_* (if dimension==1), i2_* (if dimension==2) or i3_* (if dimension==3) constants in wlsqm2_defs.
#
#         If dimension==1:
#
...
#         i2_Y4
#
#         For dimension==3, see wlsqm2_defs.pxd (there are 35 constants for the 3D case).
#
...
def interpolate_fit( xi, double[::1] fi, int dimension, int order, x, int diff=0 ):
    ...
    # TODO/FIXME: We don't have a Case instance here, since we are coming from the Python level.
    # TODO/FIXME: Now we must create one, or at least populate the required fields manually.
    ...
    with nogil:
        ...
        interpolate_model_nD( &case, xManyD, x1D, &out[0], diff )

    return out


# Create a Python lambda that interpolates a fitted surrogate model to (x,y).
# (The fi[] are actually the coefficients of a polynomial.)
#
...
# fi[] must be the coefficients output by one of the fitting functions.
#
    ...
    if dimension == 3:
        def model(x, y, z):
            if np.shape(y) != np.shape(x)  or  np.shape(z) != np.shape(x):
                raise ValueError("In model() (generated by wlsqm2_eval.lambdify_fit()): x, y and z must be of the same shape; got shape(x) = %s, shape(y) = %s, shape(z) = %s" % (np.shape(x), np.shape(y), np.shape(z)))
        ...
        def model(x, y):
            if np.shape(y) != np.shape(x):
                raise ValueError("In model() (generated by wlsqm2_eval.lambdify_fit()): x and y must be of the same shape; got shape(x) = %s, shape(y) = %s" % (np.shape(x), np.shape(y)))
...
#
cdef int interpolate_model_nD( infra.Case* case, double[::view.generic,::view.contiguous] xManyD, double[::view.generic] x1D, double* out, int diff ) nogil:
    if case.dimension == 3:
        return interpolate_model_3D( case, xManyD, out, diff )
    elif case.dimension == 2:
        return interpolate_model_2D( case, xManyD, out, diff )
    else: # case.dimension == 1:
        return interpolate_model_1D( case, x1D, out, diff )
...
# diff  : in, one of the i3_* constants in wlsqm2_defs.
#
# Note that the same xi and order must be passed here as for the model fitting.
# fi[] must contain the output the model fitting.
#
cdef int interpolate_model_3D( infra.Case* case, double[::view.generic,::view.contiguous] x, double* out, int diff ) nogil:
    DEF onesixth = 1./6.
    DEF one24th  = 1./24.
    ...
    if diff == defs.i3_F_c:
        return poly.evaluate_taylor_expansion_3D( order, case.fi, case.xi, case.yi, case.zi, x, out )
    ...
    if diff == defs.i3_X_c:
        ...
        return poly.evaluate_polynomial_3D( order-1, fi2, case.xi, case.yi, case.zi, x, out )
    ...
    # if we get here, model order >= 2
        ...
        return poly.evaluate_polynomial_3D( order-2, fi2, case.xi, case.yi, case.zi, x, out )
    ...
    # if we get here, model order >= 3
        ...
        return poly.evaluate_polynomial_3D( order-3, fi2, case.xi, case.yi, case.zi, x, out )
    ...
    # if we get here, model order >= 4
        ...
        return poly.evaluate_polynomial_3D( order-4, fi2, case.xi, case.yi, case.zi, x, out )
...
# out   : out, function values. Must be allocated by caller. Must have as nk elements (one element for each point x).
# diff  : in, one of the i2_* constants in wlsqm2_defs.
#
# Note that the same xi and order must be passed here as for the model fitting.
# fi[] must contain the output the model fitting.
#
cdef int interpolate_model_2D( infra.Case* case, double[::view.generic,::view.contiguous] x, double* out, int diff ) nogil:
    DEF onesixth = 1./6.
    DEF one24th  = 1./24.
    ...
    # Interpolate function value? (the most common case)
    if diff == defs.i2_F_c:
        return poly.evaluate_taylor_expansion_2D( order, case.fi, case.xi, case.yi, x, out )
    ...
    if diff == defs.i2_X_c:
        ...
        return poly.evaluate_polynomial_2D( order-1, fi2, case.xi, case.yi, x, out )
    ...
    # if we get here, model order >= 2
        ...
        return poly.evaluate_polynomial_2D( order-2, fi2, case.xi, case.yi, x, out )
    ...
    # if we get here, model order >= 3
        ...
        return poly.evaluate_polynomial_2D( order-3, fi2, case.xi, case.yi, x, out )
    ...
    # if we get here, model order >= 4
        ...
        return poly.evaluate_polynomial_2D( order-4, fi2, case.xi, case.yi, x, out )
...
# out   : out, function values. Must be allocated by caller. Must have as nk elements (one element for each point x).
# diff  : in, one of the i1_* constants in wlsqm2_defs.
#
# Note that the same xi and order must be passed here as for the model fitting.
# fi[] must contain the output from the model fitting.
#
cdef int interpolate_model_1D( infra.Case* case, double[::view.generic] x, double* out, int diff ) nogil:
    DEF onesixth = 1./6.
    DEF one24th  = 1./24.
    ...
    # Interpolate function value? (the most common case)
    if diff == defs.i1_F_c:
        return poly.evaluate_taylor_expansion_1D( order, case.fi, case.xi, x, out )
    ...
    if diff == defs.i1_X_c:
        ...
        return poly.evaluate_polynomial_1D( order-1, fi2, case.xi, x, out )
    ...
    # if we get here, model order >= 2
        ...
        return poly.evaluate_polynomial_1D( order-2, fi2, case.xi, x, out )
    ...
    # if we get here, model order >= 3
        ...
        return poly.evaluate_polynomial_1D( order-3, fi2, case.xi, x, out )
    ...
    # if we get here, model order >= 4
        ...
        return poly.evaluate_polynomial_1D( order-4, fi2, case.xi, x, out )
